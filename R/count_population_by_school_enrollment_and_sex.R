...
#' district) and the observation year period. The count is sub-divided by the
#' school enrollment group and sex category.
#' 
...
#'    identified by the observation year, the school enrollment group and the
#'    sex category.
#'     
#' @export
#' @examples
...
#' count_population_by_school_enrollment_and_sex(zips)
#' 
#' # Count the population from 2012 to 2015
#' count_population_by_school_enrollment_and_sex(zips, start_year=2012, end_year=2015)
#' 
#' # Count the population in 2012
#' count_population_by_school_enrollment_and_sex(zips, year=2012)
#' 
#' # Count the population in the state of California
#' count_population_by_school_enrollment_and_sex(c("California")) # State name
#' count_population_by_school_enrollment_and_sex(c("CA")) # State code
#' count_population_by_school_enrollment_and_sex(c("06")) # 2-digit state FIPS code
count_population_by_school_enrollment_and_sex <- function(geo_names,
  ...
  statvar_map <- list()
  extra_group <- "NotEnrolledInSchool"
  for (school_enrollment in c(CENSUS_SCHOOL_ENROLLMENT_GROUPS, extra_group)) {
    for (sex_category in CENSUS_SEX_CATEGORIES) {
      statvar_map[[school_enrollment]][[sex_category]] <- 
        sapply(CENSUS_PERSON_WITH_SCHOOL_EDUCATION_AGE_GROUPS,
               function(x) {
                  if (x != extra_group) {
                    paste0("Count_Person_", x, "_", sex_category, "_EnrolledIn", school_enrollment)
                  } else {
                    paste0("Count_Person_", x, "_", sex_category, "_", school_enrollment)
                  }
               },
